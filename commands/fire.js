function fire(message, command, db, handleLevelUp) {
  if (command === "fire") {
    db.get(
      `SELECT level, health, wood FROM users WHERE id = ?`,
      [message.author.id],
      (err, row) => {
        if (err) {
        ...
        db.run(
          `UPDATE users SET health = ?, wood = wood - 5 WHERE id = ?`,
          ...
          function (err) {
            ...
            message.channel.send(
              `You create a fire using 5 wood. You sit by the fire to relax. You gain ${healthGain} health. Your health is now ${newHealth}/${maxHealth}.`
            );
          }
        );
      }
