      ...
      db.get(
        ...
        (err, user) => {
          ...
          const successChance = Math.min(
            75,
            Math.max(25, 10 + user.stealth - target.stealth)
          );
          if (Math.random() * 100 < successChance) {
            ...
            db.run(
              ...
              function (err) {
                ...
                db.run(
                  `UPDATE users SET gold = gold + ?, stealth = stealth + 1 WHERE id = ?`,
                  ...
                  function (err) {
                    ...
                    message.channel.send(
                      `You successfully pickpocketed ${amount} gold from ${target.username} and gained 1 stealth.`
                    );
